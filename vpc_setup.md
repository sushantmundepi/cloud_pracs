...
This document walks you through the process of building a structured AWS VPC network consisting of both public and private subnets. The setup includes:

- A single Virtual Private Cloud (VPC) as the core network.
- Deployment across two Availability Zones: referred to as **Zone A** and **Zone B**. 
- Each zone hosts:
   - One public subnet for internet-facing resources.
   - One private subnet for internal resources. 
   - **Zone A** includes: Public Subnet 1 and Private Subnet 1
   - **Zone B** includes: Public Subnet 2 and Private Subnet 2
- **Internet Gateway (IGW)** is attached to the VPC, allowing internet access for resources in public subnets.
- **Virtual Private Gateway (VPG)** enables communication between the private subnets, supporting internal connectivity without exposing them to the internet.
